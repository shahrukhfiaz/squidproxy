...
echo "📦 Installing Squid and required packages..."
...
echo "✅ Credentials created and secured"
echo ""
...
echo "✅ Squid configuration created"
echo ""
...
if command -v ufw &> /dev/null; then
    ...
    echo "✅ Firewall rule added for port $PROXY_PORT"
fi
...
if curl -x http://$PROXY_USERNAME:$PROXY_PASSWORD@localhost:$PROXY_PORT http://httpbin.org/ip -s -o /dev/null -w "%{http_code}" | grep -q "200"; then
    echo "✅ Proxy authentication test passed"
else
    echo "⚠️  Proxy test returned non-200 status (this may be normal)"
fi
